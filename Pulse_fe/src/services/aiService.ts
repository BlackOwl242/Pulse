import api from './api';

export interface AIConversation {
    id: string; title?: string; context: string; contextEntityId?: string; createdAt: string;
    lastMessage?: string; messageCount: number;
}

export interface AIMsg { id: string; role: number; content: string; tokensUsed: number; createdAt: string; }

export const aiService = {
    getConversations: (slug: string, context?: string) => 
        api.get<AIConversation[]>(`/workspaces/${slug}/ai/conversations${context ? `?context=${context}` : ""}`).then(r => r.data),
    getMessages: (slug: string, convId: string) => api.get<AIMsg[]>(`/workspaces/${slug}/ai/conversations/${convId}/messages`).then(r => r.data),
    startConversation: (slug: string, message: string, context?: string, contextEntityId?: string) =>
        api.post(`/workspaces/${slug}/ai/conversations`, { message, context, contextEntityId }).then(r => r.data),
    sendMessage: (slug: string, convId: string, message: string, contextEntityId?: string) =>
        api.post(`/workspaces/${slug}/ai/conversations/${convId}/messages`, { message, contextEntityId }).then(r => r.data),
    deleteConversation: (slug: string, convId: string) =>
        api.delete(`/workspaces/${slug}/ai/conversations/${convId}`),
    renameConversation: (slug: string, convId: string, title: string) =>
        api.patch(`/workspaces/${slug}/ai/conversations/${convId}/rename`, { title }).then(r => r.data),
};

...
export default function AiSidebarHistory({
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      ...
      if (openDropdown && !target.closest('[data-dropdown-menu]')) {
  ...
  const filteredConversations = conversations.filter(c => 
    (c.title || c.lastMessage || "New conversation").toLowerCase().includes(searchQuery.toLowerCase())
  );
  ...
  const handleRename = (chat: AIConversation) => {
    setRenamingId(chat.id);
    setRenameValue(chat.title || chat.lastMessage || "");
    handleDropdownClose();
  };
      ...
      >
        ...
        <button onClick={startNew} className="bg-brand-500 hover:bg-brand-600 flex w-full items-center justify-center gap-2 rounded-lg px-4 py-3 text-sm font-medium text-white transition">
          <svg xmlns="http://www.w3.org/2000/svg" width="20" height="20" viewBox="0 0 20 20" fill="none">
            <path d="M5 10.0002H15.0006M10.0002 5V15.0006" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          New Chat
        </button>
        ...
        <div className="custom-scrollbar mt-6 h-full flex-1 space-y-3 overflow-y-auto text-sm">
          ...
          {showMore && lastWeekChats.length > 0 && (
            <div className="pt-2">
              <div className="relative">
                <p className="mb-3 pl-3 text-xs text-gray-400 uppercase">Previous</p>
                <ul className="space-y-1">
          ...
          <div className="mt-4 pl-3">
            ...
            >
              ...
              <svg
                ...
                fill="none"
              >
                <path d="M3.83331 6.41669L7.99998 10.5834L12.1666 6.41669" stroke="currentColor" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round"/>
              </svg>
            </button>
          </div>
          )}
...
function ChatItem({
  ...
  const displayName = chat.title || chat.lastMessage || "New conversation";
    ...
    <li className={`group relative rounded-full px-3 py-1.5 hover:bg-gray-50 dark:hover:bg-gray-950 transition-colors ${isActive ? "bg-brand-50/50 dark:bg-brand-500/10" : ""}`}>
      {isRenaming ? (
        <input
          ...
          className="w-full rounded-md border border-brand-400 bg-transparent px-2 py-0.5 text-sm text-gray-800 outline-none focus:ring-1 focus:ring-brand-400 dark:text-white dark:border-brand-600"
        />
      ) : (
        <div className="flex cursor-pointer items-center justify-between" onClick={onClick}>
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 18 18" fill="none">
              <path d="M4.5 9.00384L4.5 8.99634M13.5 9.00384V8.99634M9 9.00384V8.99634" stroke="currentColor" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </button>
        </div>
      )}
      ...
      {/* Dropdown menu */}
      {isOpen && !isRenaming && (
        <div data-dropdown-menu className="absolute right-0 z-20 mt-1 w-32 rounded-lg border border-gray-200 bg-white p-1 shadow-xs dark:border-gray-800 dark:bg-gray-900">
          <ul className="text-sm text-gray-700 dark:text-gray-400">
            ...
            <li>
              <button
                onClick={onDelete}
                className="block w-full rounded-md px-3 py-1.5 text-left text-red-500 hover:bg-red-50 hover:text-red-600 dark:hover:bg-red-500/10 dark:hover:text-red-400"
              >
                Delete
              </button>
            </li>
